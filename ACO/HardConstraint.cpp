...
bool HardConstraint::only_one_shift_per_day(int employee) {
	...
	for (int d = 0; d < _no_days; ++d) {
		tmp = &ptr[_d_j(d)];
		s = tmp[EARLY] + tmp[DAY] + tmp[LATE] + tmp[NIGHT] + tmp[REST];
		if (s != 1) return false;
	}
...
bool HardConstraint::exceed_hours(int employee) {
	...
	if (_no_weeks > 0) {
		...
		if (employee < 12) return (e36 <= s && s <= (e36 + 4));
		else if (employee == 12) return (e32 <= s && s <= (e32 + 4));
		else return (e20 <= s && s <= (e20 + 4));
	}
	return true;
}
...
bool HardConstraint::weekends_off(int employee) {
	...
	if (s < 4) return false; // 2 weekends * 2 rest
