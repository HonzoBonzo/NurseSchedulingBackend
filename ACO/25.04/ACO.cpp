	...
	for (int it = 1; it <= iterations; ++it) {
		...
		do {
			...
			for (int ant = 0; ant < _no_ants; ++ant) {
				...
				// Update employee allocation table:
				//_update_allocations(_ants[ant]);
			}
			std::cout << "hards: " << h << "                \r";
		...
		std::cout << "Found solution after "
			<< (double(clock() - clocks) / double(CLOCKS_PER_SEC)) << " [s]"
			<< ", total cost: " << _total_cost() 
			<< ", best cost: " << _best_total_cost << std::endl;

		//print_solution();
		//print_pheromones();

		// Renew allocation table:
		//_clear_allocations();
	}
	return;
}
...
void ACO::prepare(const char* filename) {
	...
	std::cout << "Count of broken hard constraints: " << _hard_constraint_validation() << std::endl;

	_update_pheromones();
	_remember_best();

	std::cout << "Total cost: " << _total_cost() << ", best cost: " << _best_total_cost << std::endl;
}
